...
function scene:create( event )
    ...
    local burningSnowflake = display.newImage("images/burningsnowflake(shadow).png")
    burningSnowflake.x = display.contentWidth/2 + 635
    burningSnowflake.y = display.contentHeight/2 - 900

    sceneGroup:insert( burningSnowflake )

    local fromLove = display.newImage("images/fromlove(shadow).png")
    fromLove.x = display.contentWidth/2
    fromLove.y = display.contentHeight/2 - 650

    sceneGroup:insert( fromLove )

    local dream = display.newImage("images/dream(shadow).png")
    dream.x = display.contentWidth/2 + 650
    dream.y = display.contentHeight/2 + 850
    ...
    snowflake.y = display.contentHeight/2 + 100

    function changeScene( event )
      if ( event.phase == "began" ) then

      elseif ( event.phase == "ended" ) then
              composer.gotoScene("scene5", {effect = "crossFade", time = 2500})
